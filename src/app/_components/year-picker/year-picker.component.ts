...
@Component({
  ...
  export class YearPickerComponent
    ...
    set max(max: number | Date) {
      if (max) {
        const momentDate =
          typeof max === 'number' ? dayjs().year(max) : dayjs(max);
    ...
    set min(min: number | Date) {
      if (min) {
        const momentDate =
          typeof min === 'number' ? dayjs().year(min) : dayjs(min);
    ...
    @ViewChild(MatDatepicker) _picker: MatDatepicker<dayjs.Dayjs>;
    ...
    _takeFocusAway = (datepicker: MatDatepicker<dayjs.Dayjs>) => { };
    ...
    writeValue(date: Date): void {
      if (date && this._isYearEnabled(date.getFullYear())) {
        ...
        if (momentDate.isValid()) {
          momentDate.date(1);
          this._inputCtrl.setValue(dayjs(date), { emitEvent: false });
        }
    ...
    _yearSelectedHandler(chosenDate: dayjs.Dayjs, datepicker: MatDatepicker<dayjs.Dayjs>): void {
      datepicker.close();
    ...
    _openDatepickerOnClick(datepicker: MatDatepicker<dayjs.Dayjs>): void {
